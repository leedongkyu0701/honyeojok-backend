...
export const mookhoSpots: SpotSeedData[] = [
...

  {
    ...
    description:
      '망상해변은 길이 5킬로미터에 달하는 넓은 모래 해변이라 걷는 재미가 좋은 곳이에요:contentReference[oaicite:0]{index=0}. 모래사장이 넓고 평탄해 맨발로도 편안하게 걸을 수 있고, 해파랑길 34코스에 속해 있어 걸으면서 작은 포구와 마을 풍경도 엿볼 수 있어요. 사람 많은 구간을 벗어나 조금만 옆으로 가면 훨씬 한적해 혼자 산책하기 좋고, 날씨가 맑은 날에는 바다 색이 안정적으로 찍혀 사진도 잘 나옵니다. 해변 뒤에는 카페나 식당으로 이어지는 길이 있어 하루 일정에 넣기 편합니다.',
    honyeoTip:
...

  {
    ...
    description:
      '묵호우동은 항구 근처에 자리한 작은 우동집이에요. 홀에 6개 안팎의 테이블만 있어 혼자 방문해도 자리 잡기가 수월하고, 주문과 반찬은 셀프 방식이라 흐름이 빠른 편입니다:contentReference[oaicite:1]{index=1}. 국물은 맑고 진하며 유자향이 나는 냉우동부터 따끈한 튀김 우동까지 메뉴가 단순해 실패할 가능성이 적어요. 어항이나 등대 쪽 동선 사이에 들르기 쉬워 산책 중 가볍게 한 끼 해결하기 좋고, 오전 시간이 한산해 웨이팅 걱정도 덜합니다.',
    honyeoTip:
...

  {
    ...
    description:
      '다이버구이는 숯불에 구운 생선 한 상을 맛볼 수 있는 로컬 식당이에요. 기본 세트에는 꽁치·가자미·고등어 등 여러 생선이 함께 나와 다양하게 맛볼 수 있고, 뜨거운 돌 위에 올려져 있어 식사 내내 따뜻하게 유지됩니다:contentReference[oaicite:2]{index=2}. 혼자 여행객을 위한 1인 메뉴가 있어 혼밥이 자연스러우며, 반찬과 매운탕도 든든하게 나와 배부르게 먹기 좋아요. 인기가 많아 점심시간에는 대기가 생기기도 하는데, 브레이크타임 직후나 이른 저녁 시간에 방문하면 비교적 한적합니다.',
    honyeoTip:
...

  {
    ...
    description:
      '비비소는 항구와 논골담길 사이에 위치한 로컬 식당으로, 육회비빔밥과 소고기짬뽕국이 대표 메뉴예요:contentReference[oaicite:3]{index=3}. 육회비빔밥은 신선한 육회를 채소와 함께 비벼 먹는 방식으로 한 그릇이 푸짐하고, 곁들이로 따뜻한 소고기국이 나와 속을 든든하게 합니다. 메뉴 구성이 단순해 주문이 빠르고, 혼밥 손님이 많아 혼자 방문해도 눈치를 볼 필요가 없어요. 아침부터 문을 열어 바다 코스에 앞서 든든한 식사를 하기에 좋고, 항구·논골담길과도 가까워 일정에 자연스럽게 넣을 수 있습니다.',
    honyeoTip:
...

  {
    ...
    description:
      '묵호김밥은 항구와 역 근처에 있는 작은 분식집이에요. 김밥 한 줄에 달걀과 속재료를 듬뿍 넣어 맛과 식감이 풍부하고, 어묵 꼬치(핫바) 등 함께 곁들일 수 있는 간식도 준비돼 있어요:contentReference[oaicite:4]{index=4}. 포장이 자연스러워 바다나 시장에 들고 가서 먹기 편하고, 혼자 방문해도 주문이 빠른 편입니다. 인기가 많아 재료가 일찍 소진될 수 있으니 오전이나 이른 점심에 방문하면 원하는 메뉴를 받을 수 있어요.',
    honyeoTip:
...

  {
    ...
    description:
      '대우칼국수는 오래된 건물 2층에 자리한 칼국수집으로, 메뉴는 오직 하나인 장칼국수예요:contentReference[oaicite:5]{index=5}. 진득한 고추장 육수에 면을 말고 각종 야채를 듬뿍 올린 방식으로, 맵지 않아 속이 부담되지 않습니다. 오래된 창문 사이로 햇살이 들어오는 내부는 동네 사람들의 일상적인 식사 공간이라 혼자 앉아도 편안하고, 주문과 식사 흐름이 빠른 편이에요. 영업 시간이 짧아 오후 늦게는 재료가 소진되어 문을 닫는 경우가 있어요:contentReference[oaicite:6]{index=6}.',
    honyeoTip:
...

  {
    ...
    description:
      '라운드어바웃은 묵호항 근처에 자리한 아담한 카페예요. 내부에는 4개 정도의 테이블만 있어 공간이 크진 않지만 창가로 들어오는 햇살 덕분에 따뜻한 분위기가 감돌고, 혼자 조용히 커피를 즐기기에 어색하지 않습니다:contentReference[oaicite:7]{index=7}. 시그니처인 흑임자 커피는 고소한 흑임자와 에스프레소의 조합이 부드럽고, 위에 얹은 크림을 중간에 섞으면 또 다른 맛을 느낄 수 있어요:contentReference[oaicite:8]{index=8}. 휘낭시에나 크로칸트 같은 디저트도 준비되어 있지만 인기 품목은 오전에 품절되기도 하니 일찍 가야 맛볼 수 있어요:contentReference[oaicite:9]{index=9}. 항구와 논골담길 사이 동선이 가까워 산책 후 들르기에 좋습니다.',
    honyeoTip:
      '혼자 방문한다면 오픈 직후(10시경)나 늦은 오후(4~5시경)가 가장 한적해요:contentReference[oaicite:10]{index=10}. 흑임자 커피는 처음에는 섞지 않고 한 모금 마신 후, 얼음과 함께 가볍게 저어 마시면 고소한 맛과 단맛을 두 번 즐길 수 있어요:contentReference[oaicite:11]{index=11}. 주말에는 작은 공간이 금방 만석이 되니 대기 시간을 감안하고 방문하세요.',
...

  {
    ...
    description:
      '한결은 어달해변 건너편에 자리한 베이커리 겸 브런치 카페로, 바다를 바라볼 수 있는 통창이 가장 큰 매력이에요:contentReference[oaicite:12]{index=12}. 건물은 3층까지 앉을 수 있고 각 층의 창가 쪽 자리를 넉넉히 배치해 바다를 바라보며 천천히 앉아 있기 좋아요:contentReference[oaicite:13]{index=13}. 브리오슈 프렌치토스트나 불고기버섯크림리조또 같은 브런치 메뉴가 든든하게 나와 한 끼 식사로도 충분하고, 커피는 디카페인으로도 주문할 수 있어 취향에 맞게 선택할 수 있어요:contentReference[oaicite:14]{index=14}. 사진을 찍기 좋은 포토존과 거울, 넉넉한 테이블 간격 덕분에 혼자 앉아도 주변의 시선을 크게 느끼지 않습니다:contentReference[oaicite:15]{index=15}.',
    honyeoTip:
      '노을이 지기 1시간 전쯤 방문하면 창 너머 바다와 하늘의 색이 변하는 모습을 여유롭게 감상할 수 있어요:contentReference[oaicite:16]{index=16}. 3층 좌석은 바다와 바로 맞닿은 듯한 시야를 제공하니, 엘리베이터가 없다면 조금 올라가서라도 창가에 자리잡아 보세요:contentReference[oaicite:17]{index=17}. 점심시간 직후에는 한적해 혼자 브런치와 커피를 즐기기 좋습니다.',
    category: SpotCategory.CAFE,
    isRecommended: true,
...

  {
    ...
    description:
      '카라멜스테이션은 카페와 호텔이 함께 있는 붉은 벽돌 건물로, 묵호역과 묵호항 사이에 자리해 있어요:contentReference[oaicite:18]{index=18}. 카페 내부는 널찍하고 창가에서 햇볕이 들어와 힙한 감성 속에서도 따스함을 느낄 수 있고, 지하층에는 1인석이 마련돼 조용히 작업하거나 책을 읽기에도 좋아요:contentReference[oaicite:19]{index=19}:contentReference[oaicite:20]{index=20}. 대표 메뉴는 콘수수커피와 카라멜크림커피로, 옥수수맛이 고소하게 느껴지며 디저트는 매장에서 직접 구워내는 휘낭시에, 바스크 치즈케이크, 쿠키 등이 다양하게 준비돼요:contentReference[oaicite:21]{index=21}:contentReference[oaicite:22]{index=22}. 카페와 호텔이 함께 있어 숙박객이 없는 시간대에는 상대적으로 한산해 혼자 앉아 바다를 바라보기 좋습니다.',
    honyeoTip:
      '영업시간은 오전 11시부터 오후 6시쯤까지니 늦은 시간에는 이용이 어려워요:contentReference[oaicite:23]{index=23}. 평일 낮이나 해 지기 전 1~2시간 전이 가장 여유롭고, 바다를 산책한 후 들어오면 자리 경쟁을 피할 수 있어요. 콘수수커피는 양이 많지 않으니 디저트와 함께 주문해보는 것도 좋고, 지하 1인석을 활용하면 혼자만의 시간을 편하게 즐길 수 있어요:contentReference[oaicite:24]{index=24}.',
    category: SpotCategory.CAFE,
    isRecommended: true,
  ...
  },
  {
    ...
    description:
      '알마레는 한섬해변 바로 앞에 위치한 다층 건물로, 1층은 주차장, 3층은 레스토랑, 4층은 디저트 카페, 5층은 루프탑으로 구성돼 있어요:contentReference[oaicite:25]{index=25}. 4층 카페는 통창과 루프탑 덕분에 바다를 한눈에 바라볼 수 있고, 커피 한 잔 가격도 크게 비싸지 않아 부담이 적습니다:contentReference[oaicite:26]{index=26}. 아메리카노를 주문하면 미니 쿠키가 함께 나오고, 크로플을 바나나에 설탕을 입혀 토치로 구워주는 등 디저트의 퀄리티가 높아요:contentReference[oaicite:27]{index=27}. 건물 규모가 커 좌석이 넉넉하고 주차 공간도 충분해 혼자 찾는 여행자도 편하게 머물 수 있습니다.',
    honyeoTip:
      '루프탑은 날씨가 좋을 때 바다와 하늘을 동시에 즐길 수 있는 최고의 자리라 해 질 무렵에 올라가 보는 것을 추천해요:contentReference[oaicite:28]{index=28}. 주문한 음료는 포장해 한섬해변을 걸으며 마셔도 좋고, 실내에서 창가 자리를 선택하면 바람이 불어도 편하게 바다를 감상할 수 있어요:contentReference[oaicite:29]{index=29}. 카페와 레스토랑의 운영시간이 다르니 방문 전 시간을 확인하세요.',
    category: SpotCategory.CAFE,
    isRecommended: true,
...

  {
    ...
    description:
      '사담은 북평시장 근처 2층에 자리한 홍콩식 와인바로, 외관부터 홍콩 레트로 감성이 물씬 풍겨요:contentReference[oaicite:30]{index=30}. 내부는 은은한 조명과 와인 병으로 꾸며져 있어 분위기가 차분해 혼자 앉아도 어색하지 않으며, 바와 아늑한 테이블들이 적당한 거리감으로 배치돼 있어요:contentReference[oaicite:31]{index=31}. 메뉴는 쪽파 & 베이컨 크림치즈 크로켓, 홍콩식 오이무침 등 흔치 않은 안주와 와인 리스트가 준비돼 있어 새로운 조합을 즐길 수 있고, 직원들이 추천을 도와줘 와인 초보자도 편하게 주문할 수 있습니다:contentReference[oaicite:32]{index=32}. 정기 휴일과 영업 시간이 있으니 방문 전에 확인해야 합니다.',
    honyeoTip:
      '혼술이라면 평일 오픈 직후나 장날 오전시간에 방문하면 여유로운 분위기를 즐길 수 있어요:contentReference[oaicite:33]{index=33}. 외관이 다소 눈에 띄지 않지만 2층으로 올라가면 홍콩 영화 속 한 장면 같은 분위기가 펼쳐지니 주저하지 말고 문을 열어보세요. 귀가 동선을 미리 정해두고 1~2잔 정도 가볍게 즐기면 여행 마무리로 딱입니다.',
...

  {
    ...
    description:
      '바람의 언덕 동해타파스 와인 앤 비어는 논골담길 언덕 위에 자리한 작은 와인·맥주 바예요. 길을 따라 한참 올라가야 하지만 정상에 도착하면 바다와 항구가 한눈에 내려다보이는 풍경이 펼쳐져서 오르는 수고를 잊게 해줍니다:contentReference[oaicite:34]{index=34}. 야외 좌석은 비닐막이 씌워진 바 좌석 5석 정도와 실내 온실처럼 꾸며진 테이블 몇 개가 있어 바다를 가까이서 바라보며 한 잔을 즐길 수 있어요:contentReference[oaicite:35]{index=35}. 메뉴는 치즈와 나초, 빠네수프 등 간단히 곁들일 수 있는 안주와 맥주·와인이 준비돼 있고, 날씨에 따라 난로를 켜주어 겨울에도 따뜻하게 머물 수 있습니다:contentReference[oaicite:36]{index=36}. 카페 느낌보다는 소박한 펜션 같은 분위기라 혼자 가도 부담이 적어요.',
    honyeoTip:
      '오르막길이 제법 가파르니 편한 신발을 신고 천천히 올라오세요. 노을이 지는 시간대에 방문하면 해가 지는 바다와 어둑한 항구를 이어서 볼 수 있어 분위기가 가장 좋습니다:contentReference[oaicite:37]{index=37}. 야외 좌석은 정면으로 해가 들어와 선글라스가 있으면 편하고, 겨울엔 따뜻한 겉옷과 장갑을 챙기면 더 오래 머물 수 있어요:contentReference[oaicite:38]{index=38}.',
    category: SpotCategory.DRINK,
    isRecommended: true,
...

  {
    ...
    description:
      '125TH 스트릿은 천곡동의 상가거리 안쪽에 자리한 와인·칵테일 바예요. 예전에는 술집으로 운영되던 공간을 칵테일 바로 개조해 차분한 분위기를 갖추고 있고, 메인홀에는 둥근 테이블과 바 좌석이 적당히 섞여 있어 혼자 앉아도 부담스럽지 않아요:contentReference[oaicite:39]{index=39}. 카펫이 깔린 바닥과 은은한 조명, 소품들이 몽환적인 무드를 만들어주며, 메뉴에는 위스키·와인·리큐르를 기반으로 한 칵테일과 무알콜 칵테일까지 다양하게 준비돼 있어 취향에 맞는 한 잔을 고를 수 있습니다:contentReference[oaicite:40]{index=40}. 기본 안주로 제공되는 설탕 건빵 외에도 감자튀김 등 간단한 안주를 주문할 수 있어 가볍게 술 한 잔하며 시간을 보내기 좋아요:contentReference[oaicite:41]{index=41}.',
    honyeoTip:
      '영업시간은 월요일부터 토요일까지 오후 6시부터 새벽 2시까지예요:contentReference[oaicite:42]{index=42}. 혼자라면 이른 시간대에 들어가 바텐더와 천천히 대화를 나누며 자신에게 맞는 칵테일을 추천받아 보세요. 분위기가 차분해 시끌벅적한 주점을 선호하지 않는 사람에게 잘 맞고, 귀가 동선을 미리 정해두면 마음이 한결 편합니다.',
    category: SpotCategory.DRINK,
    isRecommended: true,
