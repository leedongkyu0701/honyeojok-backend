...
import { FindHotSpotsResponseDto } from './dto/response/find-hot-spots.response.dto';
...
export class SpotsService {
  ...
  async findByQuery(
    ...
    region: string,
  ): Promise<{ data: SpotCardResponseDto[]; totalPages: number }> {
  ...
  async findHot(takePerCategory = 10): Promise<FindHotSpotsResponseDto> {
    ...
    const response: FindHotSpotsResponseDto = {
      food: [],
      cafe: [],
      drink: [],
      activity: [],
  ...
  async findNearbyByPoints(args: FindNearbyByPointsArgs) {
    ...
    const results: Partial<Record<SpotCategory, SpotMapResponseDto[]>> = {};
    for (const c of cats) {
